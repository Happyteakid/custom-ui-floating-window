...
const AddProduct = () => {
  ...
  const renderProductField = (rowData, field) => {
    ...
    if (field.field_type === 'enum' && options.length > 0) {
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      ...
      <DataTable value={otherProducts} paginator rows={5}>
        {productFields.map((field, index) => (
          <Column key={index} field={field.key} header={field.name} body={(rowData) => renderProductField(rowData, field)} sortable filter></Column>
        ))}
      </DataTable>
      <>
